...
def ParametersProcessing(material, inpname, UMATname, u, l, RFarea, d, identification_type,
    ...
    test1, Excurve, true3, Mns, truecurve, slope, m = ParameterCurve(SScsv_name, EE, bline, averageformer,
                                                                     averagelatter)
...
def Tensileinpmaker(nn, parameters_instance):
    ...
    data0004 = data0[b1:]
    with open(f"plastic_n_{nn}.csv", mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(fitcurve)
...
def Tensileendlog(nn, parameters_instance):
    ...
    while not fin and not fin2:
        ...
        sys.stdout.write(
            f"\rCase: {Case_name}{nn}, Costed Time: {elapsed_time:.2f} minutes")  # Real-time output and overwrite the previous line
        sys.stdout.flush()
    time.sleep(10)
    print("   OVER   ", end="")
...
def Tensileerrormin(nn, parameters_instance):
    ...
    output_filename = f"{Case_name}_N_{nn}_Load.csv"
    header = ["Ave.Error", "Time", "Stroke", "Experiment", "FEM"]
    data = [[""] + list(row) for row in zip(wlist[1:],
                                            dataU22,
                                            interpolated_data,
                                            dataRF22)]
    data[0][0] = error
...
def FSParametersProcessing(material, inpname, UMATname, u, l, RFarea, d, identification_type, nn,
    ...
    materialcsv_name3 = material + "_ss.csv"
...
def FSTensileerrormin(fs, parameters_instance):
    ...
    error, wlist, interpolated_data, dataU22, dataRF22 = calculate_error(data, test1, Excurve)
    output_filename = f"{Case_name}_FS_{fs}_Load.csv"
    header = ["Ave.Error", "Time", "Stroke", "Experiment", "FEM"]
    data = [[""] + list(row) for row in zip(wlist,
                                            dataU22,
                                            interpolated_data,
                                            dataRF22)]
    with open(output_filename, mode='w', newline='') as file:
...
def calculate_error(data, exp, Excurve):
    def Fuction_Excurve(U, exp, expi):
        ...
        return exRF22
    U_min, U_max = 0.0, 0.226
    data = [row for row in data if U_min <= row[0] <= U_max]
    exp = [row for row in exp if U_min <= row[0] <= U_max]
    if data[-1][0] < U_max:
        data.append([U_max, 0])
    ...
    if not os.path.isfile(material_plastic_csv_name):
        ...
        if os.path.isfile(SScsv_name):
            ...
            raise FileNotFoundError(
                "Both {} and {} and {} files do not exist".format(material_plastic_csv_name, SScsv_name, SS0csv_name))
        test1, Excurve, true3, Mns, truecurve, slope, m = ParameterCurve(SScsv_name, EE, bline, averageformer,
                                                                         averagelatter)
        fitcurve = PlasticCurve(nn, true3, slope, m, truecurve)
        with open(material_plastic_csv_name, "w", newline="") as csvfile:
...
def ArcanStep(name, material, inpname, fs, iout, thresold, incnum, finc, mininc, maxinc, numatmpinc, elementtype, u, l,
              cpus, UMATname, residualerror, identification_type):
    if os.path.exists("Data_B.pkl"):
        ...
        Arcan_name=result.Arcan_name
        LDIC_name =result.LDIC_name
        inp_name  =result.inp_name
        Case_name =result.Case_name
        uppern    =result.uppern
        ATRI      =result.ATRI
        ...
        if os.path.isfile("Results_fs.csv"):
            ...
            if ATRI_fs != ATRI_B:
                ATRI = ATRI_fs
                print(f"Change ATRI from {ATRI_B:.4f} to Result_fs.csv's value: {ATRI_fs:.4f}")
            else:
                ATRI = ATRI_B
                print(f" ATRI {ATRI_B:.4f}")
        ...
        ud0 = pd.read_csv(ld0_name)
        ld0 = pd.read_csv(ud0_name)
        if os.path.isfile("Results_fs.csv"):
            if os.path.isfile("Results_old_fs.csv"):
                with open("Results_old_fs.csv", 'r') as csvfile:
                    reader = csv.reader(csvfile)
                    next(reader)
                    data_fs = []
                    error_fs = []
                    for row in reader:
                        try:
                            ...
                            break
                    Best_fs = next(reader)
                    ATRI2 = float(Best_fs[2])
                print(f"Old ATRI from {ATRI2:.4f} to Result_fs.csv's value:")
            elif os.path.exists(os.path.join(os.path.dirname(__file__), 'fs', "Results_old_fs.csv")):
                with open(os.path.join(os.path.dirname(__file__), 'fs', "Results_old_fs.csv"), 'r') as csvfile:
                    reader = csv.reader(csvfile)
                    next(reader)
                    data_fs = []
                    error_fs = []
        ...
        # with open("mma-timelist.csv", mode='r', newline='') as file:
        #     reader = csv.reader(file)
        #     timelist = [float(row[0]) for row in reader if row]
        ...
        zerop = sc0.iloc[nupperp + nlowerp, 1:4].values.tolist()
        notcht = sc0.iloc[-1, 1:4].values.tolist()
...
def Arcaninpmaker_B(b, parameters_instance):
    ...
    inpdata[material_position + 1] = ','.join(
        inpdata[material_position + 1].split(',')[:2] + [str(A), str(b)] + inpdata[material_position + 1].split(',')[
                                                                           4:])
    A = float("{:.6g}".format(A))
...
def Arcanendlog(b, parameters_instance):
    ...
    while not fin and not fin2:
        ...
        elapsed_time = (current_time - start_time) / 60
        sys.stdout.write(
            f"\rCase: {Case_name}{A}_B_{b}, Costed Time: {elapsed_time:.2f} minutes")
        sys.stdout.flush()
    time.sleep(10)
    print("   OVER   ", end="")
...
def Arcanerrormin_B(b, parameters_instance):
    ...
    header = ["Ave.Error", "Time", "Stroke", "Experiment", "FEM"]
    data = [[""] + list(row) for row in zip(timelist[1:],  # error
                                            [(t - timelist[1]) / 30 for t in timelist[1:]],
                                            [item for sublist in Exdata2 for item in sublist],
                                            [item for sublist in RFdatas for item in sublist])]
    data[0][0] = error
    with open(output_filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(header)
...
def ParameterCurve(SScsv_name, EE, bline, averageformer, averagelatter):
    ...
    # test10 = test1  ### includes 0,0 for  ST
    ...
    trueminus = np.array(true1)

    zeroposition = np.where(trueminus[:, 0] < 0)[0][-1] + 1
    true2 = true1[zeroposition:]  ### it starts from the first point of growth
    # true2 = true1[zeroposition - 1:]  ### the second point for Material ST
    zeropoint = true2[0][0]
    true3 = np.array([[row[0] - zeropoint, row[1]] for row in true2[:-2]])
    true3 = np.array(true3)

    dydx = np.gradient(true3[:, 0]) / np.gradient(true3[:, 1])
    strictly_increasing_true3 = []
    previous_value = -np.inf
    for row in true3:
        if row[1] > previous_value:
            strictly_increasing_true3.append(row)
            previous_value = row[1]
    true3_filtered = np.array(strictly_increasing_true3)
    dydx_filtered = dydx[:len(true3_filtered)]
    trueint = CubicHermiteSpline(true3_filtered[:, 1], true3_filtered[:, 0], dydx_filtered)
...
def alldatafind(exp, expi, parameters_instance, identification_type):
    ...
    files = [file for file in os.listdir() if TypeName in file and file.endswith('.dat') and "Best" not in file]
    error = []
    Nvalue = []
    RFnode = parameters_instance.RFnode
    RFarea = parameters_instance.RFarea
    ...
    for f in files:
        ...
        if identification_type == 'n':
            ...
            data = data2
            error_value, wlist, interpolated_data, dataU22, dataRF22 = calculate_error(data, exp, expi)
            Nvalue_value = os.path.splitext(f)[0].split('_')[-1]
...
def allArcandatafind(parameters_instance):
    ...
    for f in files:
        ...
        wlist0 = [timelist[i] - timelist[i - 1] for i in range(2, len(timelist))]
        wlist = np.append(wlist0, wlist0[-1]) / (timelist[-1] - timelist[1])
        RFdatas = np.reshape(RFdatas, (-1, 1))
        Exdata2 = np.resize(Exdata2, RFdatas.shape)
        error0 = (np.array(Exdata2) - np.array(RFdatas)) ** 2
...
def move_files(subfolder_name, Best_Nvalue):
    ...
    files_to_move = [f for f in os.listdir(os.path.dirname(os.path.abspath(__file__))) if
                     TypeName in f or TypeName2 in f]
    ...
    if subfolder_name == 'fs':
        Best_fs_dat = "_fs_" + str(Best_Nvalue)
        fs_to_copy = [f for f in os.listdir(subfolder) if str(Best_fs_dat) in f]
        for file in fs_to_copy:
            source_path = os.path.join(subfolder, file)
            destination_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), file)
            shutil.copy2(source_path, destination_path)
            if "Best" not in file and ".dat" in file:
                datdata = open(file, "r", encoding='utf-8').readlines()
                triaxiality = DAT_values(datdata, 19, 2)
                eqplas = DAT_values(datdata, 13, 5)
                ATRI = average_values(triaxiality, eqplas)

                headers = ["Aver.tria", 'Triaxiality', 'EqPlas']
                data = [[""] + list(row) for row in zip(triaxiality, eqplas)]
                data[0][0] = ATRI
                Triaxiality_CSV = f"Old_triaxiality_{Best_Nvalue}.csv"
                subfolder_tria = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fs", "Triaxiality")
                os.makedirs(subfolder_tria, exist_ok=True)
                output_path = os.path.join(subfolder_tria, Triaxiality_CSV)
                with open(output_path, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(headers)
                    writer.writerows(data)
                print("                                           OldATRI", ATRI)
            if "Best" in file and ".csv" in file:
    ...
    if subfolder_name == 'n':
        ...
        for file in fs_to_copy:
            ...
            if "Best" not in file:
                ...
                data = [[""] + list(row) for row in zip(triaxiality, eqplas)]
                data[0][0] = ATRI
                Triaxiality_CSV = f"Old_tria_{Best_Nvalue}.csv"
                subfolder_tria = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fs", "Triaxiality")
                os.makedirs(subfolder_tria, exist_ok=True)
                output_path = os.path.join(subfolder_tria, Triaxiality_CSV)
                with open(output_path, mode='w', newline='') as file:
...
def exjob(x, parameters_instance):
    ...
    if identification_type == 'n':
        ...
        Nerror = find_dat_move_1(Case_dat_name, subfolder, subfolder2, Case_name, x, identification_type,
                                 parameters_instance)
    elif identification_type == 'fs':
        Case_name = parameters_instance.Case_name
        Case_dat_name = f"{Case_name}{x}.dat"
        ...
        Nerror = find_dat_move_1(Case_dat_name, subfolder, subfolder2, Case_name_x, x, identification_type,
                                 parameters_instance)
...
def terminate(x, parameters_instance):
    x = float("{:.6g}".format(x))
    print(f"\rUseless:{x}       ", end="", flush=True)
    identification_type = parameters_instance.identification_type
    ...
    if os.path.exists(Case_log_name):
        print(f"\rUseless:{x}          updating:", end="", flush=True)
        time.sleep(0.5)
        if not os.path.exists(Case_bat_name):
            with open(Case_bat_name, "w") as terminate_output:
                terminate_output.writelines(terminate023)
            time.sleep(5)
            print(f"\rUseless:{x}          生成bat", end="", flush=True)
            time.sleep(5)
        with open(Case_log_name, "r") as logfile:
            lines = logfile.readlines()
        fin = [i for i, line in enumerate(lines) if "COMPLETED" in line]
        ...
        if not fin and not fin2:
            try:
                subprocess.Popen(["cmd.exe", "/c", f"start {Case_bat_name}"], shell=True)
                print(f"\rUseless:{x}          perform bat: ", end="", flush=True)
                time.sleep(120)
            except Exception as e:
                print("Error:", e)
        ...
        time.sleep(0.5)
        for file in files_to_move:
            print(f"\rUseless:{x}          Moving {file}", end="", flush=True)
            source_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), file)
            destination_path = os.path.join(subfolder, file)
            shutil.move(source_path, destination_path)
            time.sleep(0.5)
        print(f"\rUseless:{x}          moveing finished", flush=True)
    return
...
def GoldenMethod1Parallel(parameters_instance, jobnumber):
    ...
    if jobnumber == 3:
        terminate(otherk, parameters_instance)
...
def Identification(identification_parameter, material):
    ...
    if material == 'AL':
        ...
        RFarea = 4.9
        d = 15
        print("material type unknown, using default length and area ")
...
def SDVlist_parameter():
    SDVnamelist0 = ["EELAS(1)", "EELAS(2)", "EELAS(3)", "EELAS(4)", "EELAS(5)", "EELAS(6)",
                    ...
                    "LODE ANGLE", "ID", "CRITICAL PLASTIC STRAIN", "SEQPLAS", "Damage_Index", "Frac_Index",
                    "JSLOPE", "ALODEpara", "ALODEnorma"]
...
def ABCfuction(x, fs, ATRI):
    ...
    A = fs / np.exp(-x * ATRI)
    return A
def fracture_strain_supplement():
    def find_crack_path(datname):
        keyword = "SDV13       SDV16       SDV19"
        data = open(datname, "r", encoding='utf-8').readlines()
        a = [i for i, line in enumerate(data) if keyword in line]
        path_ele = []
        frac_ID = 0
        First_fracture_ele = 0
        for j in range(len(a)):
            data_line_all = data[a[j] + 3:]
            b = [i for i, line in enumerate(data_line_all) if "MAXIMUM" in line]
            SDVV_all = data_line_all[:b[0] - 1]
            MissNum = []
            path = []
            for line in SDVV_all:
                if line.split()[1] == "1":
                    if float(line.split()[3]) == 0:
                        MissNum.append(float(line.split()[0]))
            if MissNum:
                path_ele_flat = [num for sublist in path_ele for num in sublist]
                path.extend([num for num in MissNum if num not in path_ele_flat])
                path_ele.append(path)
                if frac_ID == 0:
                    frac_ID = 1
                    First_fracture_ele = MissNum
        return First_fracture_ele

    def find_first_ATRI(datname, first_num, subfolder):
        keyword = "SDV13       SDV16       SDV19"
        data = open(datname, "r", encoding='utf-8').readlines()
        a = [i for i, line in enumerate(data) if keyword in line]
        Crack_all_ele = []
        ...
        for j in range(len(a)):
            ...
            for line in SDVV_all:
                if line.split()[1] == "1":
                    if float(line.split()[0]) == int(first_num):
                        Crack_all_ele.append(float(line.split()[0]))
                        Crack_all_sdv13.append(float(line.split()[2]))
                        Crack_all_sdv16.append(float(line.split()[3]))
                        Crack_all_sdv19.append(float(line.split()[4]))
        ATRI = average_values(Crack_all_sdv19, Crack_all_sdv13)
        headers = ["Aver.tria", 'Triaxiality', 'EqPlas']
        data = [[""] + list(row) for row in zip(Crack_all_sdv19, Crack_all_sdv13)]
        data[0][0] = ATRI
        Triaxiality_CSV = f"Tria_{first_num}.csv"
        output_path = os.path.join(subfolder, Triaxiality_CSV)
        with open(output_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(headers)
            writer.writerows(data)
    ...
    if os.path.exists("Data_fs.pkl"):
        fs_pkl_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Data_fs.pkl")
    elif os.path.exists(os.path.join("fs", "Data_fs.pkl")):
        fs_pkl_path = os.path.join("fs", "Data_fs.pkl")
    else:
        print("ERROR: No Data_fs.pkl existed. ")
    if os.path.isfile("Results_fs.csv"):
        fs_res_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Results_fs.csv")
    elif os.path.exists(os.path.join("fs", "Results_fs.csv")):
        fs_res_path = os.path.join("fs", "Results_fs.csv")
    else:
        print("ERROR: No Results_fs.csv existed. ")

    if os.path.isfile("Results_old_fs.csv"):
        print("Results_old_fs.csv already existed. ATRI re-calculating finished.")
    elif os.path.exists(os.path.join("fs", "Results_old_fs.csv")):
        print("Results_old_fs.csv already existed. ATRI re-calculating finished.")
    elif os.path.exists(os.path.join("ALLResults", "Results_old_fs.csv")):
        print("Results_old_fs.csv already existed. ATRI re-calculating finished.")
    else:
        print("Strat ATRI re-calculating!")
        with open(fs_pkl_path, 'rb') as file:
            parameters_instance = pickle.load(file)
        with open(fs_res_path, 'r') as csvfile:
            reader = csv.reader(csvfile)
            lines = list(reader)
            for i in range(len(lines)):
                if "OVER" in lines[i]:
                    j = i
                    ATRI_fs = float(lines[i - 1][2])
                    Best_result_fs = float(lines[i - 1][0])
        Case_name = parameters_instance.Case_name
        UMATname = parameters_instance.UMATname
        cpus = parameters_instance.cpus
        Best_fs_inp = Case_name + str(Best_result_fs) + ".inp"
        Case_bat_name = f"make_Best_fs_{Best_result_fs}.bat"
        Case_log_name = f"Best_fs_{Best_result_fs}.log"
        Case_dat_name = f"Best_fs_{Best_result_fs}.dat"
        if os.path.exists(Case_dat_name):
            time.sleep(1)
        else:
            data0 = open(Best_fs_inp, "r", encoding='utf-8').readlines()
            a1 = next(i for i, line in enumerate(data0) if "*EL Print,elset=center" in line)
            data0[a1] = "*EL Print\n"
            data0[a1 + 1] = "SDV13, SDV16, SDV19\n"
            a2 = next(i for i, line in enumerate(data0) if "*EL Print,elset=center" in line)
            data0[a2] = "*EL Print\n"
            data0[a2 + 1] = "SDV13,SDV16,SDV19\n"
            data0[a2 - 9] = "*Output,field,frequency=1\n"
            # calculating again
            with open(f"Best_fs_{Best_result_fs}.inp", 'w', newline='') as outfile:
                outfile.writelines(line for line in data0)
            with open("make023.bat", "r") as make023_file:
                make023 = make023_file.readlines()
            make023[5] = f"ab2023 job=Best_fs_{Best_result_fs} user={UMATname} cpus={cpus}\n"
            with open(Case_bat_name, "w") as make023_output:
                make023_output.writelines(make023)
            time.sleep(5)
            try:
        ...
        while not fin and not fin2:
            ...
            fin = [i for i, line in enumerate(loglines) if "COMPLETED" in line]
            fin2 = [i for i, line in enumerate(loglines) if "Wrap-up" in line]
            time.sleep(10)
            current_time = time.time()
            elapsed_time = (current_time - start_time) / 60
            sys.stdout.write(
                f"\rWaiting: Best_fs_{Best_result_fs}, Costed Time: {elapsed_time:.2f} minutes")
            sys.stdout.flush()
        time.sleep(10)
        sys.stdout.write(
            f"\rWaiting: Best_fs_{Best_result_fs}, Costed Time: {elapsed_time:.2f} minutes  Calculating ATRI!!!")

        first_num = find_crack_path(Case_dat_name)
        print(first_num)

        ATRISUM = 0
        sum = 0
        data_to_write = []
        subfolder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fs", "Triaxiality")
        os.makedirs(subfolder, exist_ok=True)
        for i in range(len(first_num)):
            try:
                ATRI_1 = find_first_ATRI(Case_dat_name, first_num[i], subfolder)
                data_to_write.append((first_num[i], ATRI_1))
                print(first_num[i], ATRI_1)
                ATRISUM = ATRI_1 + ATRISUM
                sum = sum + 1
            except ValueError as e:
                pass
        ATRI = ATRISUM / sum
        output_csv_path = os.path.join(subfolder, "ATRI_results.csv")
        with open(output_csv_path, mode='w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['Element', 'ATRI'])
            writer.writerows(data_to_write)
        print("    ATRI = ", ATRI)

        if ATRI_fs != ATRI:
            parameters_instance.ATRI = ATRI
            print("Change ATRI from", ATRI_fs, "to Result_fs.csv's value:", ATRI)
            with open('Data_fs.pkl', 'wb') as f:
                pickle.dump(parameters_instance, f)
            with open("Results_old_fs.csv", 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerows(lines)
            lines[j - 1] = [lines[j - 1][0], lines[j - 1][1], ATRI]
            with open("Results_fs.csv", 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerows(lines)
    return
...
def START_NFsB(identification_parameter, material, inpname, inpname2,
    ...
    if identification_type == 'n':
        ...
        Best_Nvalue, Best_Avalue, Best_error = move_files_to_subfolder(identification_type, error, Nvalue,
                                                                       [0] * len(Nvalue))
        ...
        parameters_instance = FSParametersProcessing(material, inpname, UMATname, u, l, RFarea, d, identification_type,
                                                     nn,
                                                     bline, averageformer, averagelatter, cpus, residualerror)
        ...
        parameters_instance = ArcanStep(name, material, inpname, fs, iout, thresold, incnum, finc, mininc, maxinc,
                                        numatmpinc,
...
def Effective_Element_Size(material, inpname, inpname2, UMATname, ln, un,lfs, ufs,lb, ub,cpus, jobnumber, s0, si):
    def find_inp(prefix, size):
        size_val = float(size)
        for fname in os.listdir():
            ...
            if not fname.endswith(".inp"):
                continue
            try:
                suf = fname[len(prefix) + 1: -4]  # 去掉 prefix_ 和 .inp
                if float(suf) == size_val:
                    return fname
            except ValueError:
                pass
        raise FileNotFoundError(
            f"[ERROR] Required .inp not found for '{prefix}', size='{size}'. "
        )

    def patch_allexe(dest, material, inpname_tensile, inpname_arcan, UMATname, un, ln,
                    ufs, lfs, ub, lb, cpus, jobnumber):
        allexe_path = os.path.join(dest, "ALLEXE.py")
        with open(allexe_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        i_start = None
        i_end = None
        for i, line in enumerate(lines):
            if i_start is None and "Set base parameters" in line:
                i_start = i
            if i_start is not None and "Starting Calculation" in line:
                i_end = i
                break
        if i_start is None:
            raise ValueError(f"[ERROR] Cannot find marker 'Set base parameters' in {allexe_path}")
        if i_end is None:
            raise ValueError(f"[ERROR] Cannot find marker 'Starting Calculation' in {allexe_path}")
        if i_end <= i_start:
            raise ValueError(f"[ERROR] Marker order invalid in {allexe_path}")
        inpname_tensile = os.path.splitext(inpname_tensile)[0]
        inpname_arcan = os.path.splitext(inpname_arcan)[0]
        new_block = f'''material = "{material}"
inpname_tensile = "{inpname_tensile}"
inpname_arcan = "{inpname_arcan}"
UMATname = "{UMATname}"
...
jobnumber = {jobnumber}'''
        patched = []
        patched.extend(lines[:i_start + 1])
        if not patched[-1].endswith("\n"):
            patched[-1] += "\n"
        patched.append("\n")
        ...
        if not new_block.endswith("\n"):
            patched.append("\n")
        patched.append("\n")
        patched.extend(lines[i_end:])
        with open(allexe_path, "w", encoding="utf-8") as f:
            f.writelines(patched)
        # print(f"[OK] Patched ALLEXE.py in {dest}")

    def copy_n_results_from_s0(s0_dir, dest):
        candidates = [
            os.path.join(s0_dir, "ALLResults", "Results_n.csv"),
            os.path.join(s0_dir, "n", "Results_n.csv"),
            os.path.join(s0_dir, "Results_n.csv"),
        ]
        for src_csv in candidates:
            if os.path.isfile(src_csv):
                shutil.copy2(src_csv, os.path.join(dest, "Results_n.csv"))
                return
        raise FileNotFoundError("[ERROR] Results_n.csv not found in s0 folder")

    src = os.getcwd()
    s0_val = float(s0)
    si_clean = [s for s in si if float(s) != s0_val]
    si = [s0] + si_clean
    sizes_list = []
    Best_n_list = []
    Best_fs_list = []
    ...
    for size in si:
        folder_name = f"Size{size}"
        dest = os.path.join(src, folder_name)
        if not os.path.exists(dest):
            os.mkdir(dest)
        print(f"[OK] Folder {folder_name} prepared")

        results_all_csv = os.path.join(dest, "Results_ALL.csv")
        if os.path.isfile(results_all_csv):
            with open(results_all_csv, "r", encoding="utf-8", errors="replace") as f:
                lines = f.readlines()
            ...
            print(f"  Best_atri  = {Best_atri}")
            print(f"  Best_a     = {Best_a}")
            print(f"  Best_b     = {Best_b}")
        else:
            for ext in ("*.csv", "*.bat", "*.f"):
                for fname in glob.glob(ext):
                    shutil.copy(fname, dest)
            tensile_inp = find_inp(inpname, size)
            arcan_inp = find_inp(inpname2, size)
            shutil.copy2(os.path.join(src, tensile_inp), dest)
            shutil.copy2(os.path.join(src, arcan_inp), dest)
            shutil.copy(os.path.join(src, "ALLEXE.py"), dest)
            shutil.copy(os.path.join(src, "ALLidentifcation.py"), dest)
            patch_allexe(
                dest, material, tensile_inp, arcan_inp, UMATname,
                ln, un, lfs, ufs, lb, ub, cpus, jobnumber)
            s0_folder = os.path.join(src, f"Size{s0}")
            if float(size) != s0_val:
                copy_n_results_from_s0(s0_folder, dest)
            print(f"=================================================")
            subprocess.run(
                [sys.executable, "ALLEXE.py"],
                cwd=dest,  # run inside subfolder
                check=True,  # raise error if ALLEXE fails
            ...
            if os.path.isfile(results_all_csv):
                ...
                raise ValueError(f"[ERROR] No {results_all_csv} ")
        print(f"[DONE] {folder_name}")
        print("-------------------------------------------------")
        sizes_list.append(size)
        Best_n_list.append(float(Best_n))
        Best_fs_list.append(float(Best_fs))
        Best_atri_list.append(float(Best_atri))
...
def Effective_Element_Size_calculate(s0, si):
    ...
    def calculate_average2(file1, file2, file3, file4, fracture_point, fracture_columns):
        ...
        df3 = pd.read_excel(file3)
        df4 = pd.read_excel(file4)
        # df1 = df1[df1.iloc[:, 0] < 260 ]      #260
        # df2 = df2[df2.iloc[:, 0] < 260 ]      #260
        # df3 = df3[df3.iloc[:, 0] < 260 ]      #260
        # df4 = df4[df4.iloc[:, 0] < 260 ]      #260
        Damage_data = df1.iloc[:fracture_point + 5, fracture_columns]  # 1:8      :fracture_point+2
        Eqplas_data = df2.iloc[:fracture_point + 5, fracture_columns]  # 1:8      :fracture_point+2
        FSSSSS_data = df3.iloc[:fracture_point + 5, fracture_columns]  # 1:8      :fracture_point+2
        Sizeee_data = df4.iloc[:fracture_point + 5, fracture_columns]  # 1:8      :fracture_point+2

        valid_rows = Sizeee_data[(Sizeee_data > 0).all(axis=1)]
        if not valid_rows.empty:  # 检查是否有满足条件的行
            Leff0 = valid_rows.iloc[0].values  # 提取第一行的数值部分
        ...
        for i in range(Size.shape[1]):  # 遍历列
            ...
            Leff0s.append(Leff0[i])

        Leff_ratio = np.mean(Leff_ratios)
        Leff_value = np.mean(Leff_values)
    ...
    def damage_time(file_path):
        ...
        # df = df[df.iloc[:, 0] < max_point]
        # df = df[df.iloc[:, 0] > max_point]
        columns_data = df.iloc[:, 1:9]
        Time = df.iloc[:, 0]

        row_index = None
        exceeding_columns = []  # 用于存储列索引
        for i, row in columns_data.iterrows():
            greater_than_one = row[row >= 1]  # 找到大于1的值
            if len(greater_than_one) >= 4:  # 判断大于1的数量是否大于等于4
                row_index = i
                exceeding_columns = [columns_data.columns.get_loc(col) + 1 for col in
                                     greater_than_one.index.tolist()]  # 记录列索引
                break
        ...
        for Size in range(len(si)):
            ...
            for line in selected_lines:
                ...
                while start_idx < len(line):
                    ...
                    row.append(content)
                    start_idx = end_idx + space_gap
                title_columns.append(row)
            Atitle_columns = ["".join(col) for col in zip(*title_columns)]
            Atitle_columns = [re.sub(r'\s{2,}', ' ', col) for col in Atitle_columns]
            # print(Atitle_columns)
            ...
            for columns in data:
                ...
                if tuple_columns not in seen:
                    unique_data.append(columns)
                    seen.add(tuple_columns)
            unique_data = unique_data[:-2]
            num_columns = len(unique_data[0])
            print(num_columns)

            first_column = [row[0] for row in unique_data]
            for i in range(1, num_columns, 8):
                current_titles = Atitle_columns[i]
                current_group_titles = Atitle_columns[i:i + 8]
                match_E = re.search(r"E\s*[:]?\s*(\d+)", current_titles)
                match_N = re.search(r"N\s*[:]?\s*(\d+)", current_titles)
                if match_E and match_N:
                    E_number = int(match_E.group(1))
                    N_number = int(match_N.group(1))
                    if all(f"{E_number}" in title for title in current_group_titles):
                        E_number = E_number
                    else:
                        raise ValueError(f"Error:E_number = {E_number},")
                else:
                    raise ValueError(f"Error: Element wrong {i} to {i + 7}")

                if all("CRITICAL" in title for title in current_group_titles):
                    Type = "FS"
                elif all("DAMAGE" in title for title in current_group_titles):
                    Type = "Damage"
                elif all("EQPLAS" in title for title in current_group_titles):
                    Type = "Eqplas"
                elif all("_ELEM" in title for title in current_group_titles):
                    Type = "Size"
                elif all("FELEM" in title for title in current_group_titles):
                    Type = "FSIZE"
                elif all("ANCE" in title for title in current_group_titles):
                    Type = "DIS"
                elif all("TRI" in title for title in current_group_titles):
                    Type = "TRI"
                else:
                    raise ValueError(f"Error: Group {i} to {i + 7} does not belong ")

                grouped_data = [row[i:i + 8] for row in unique_data]
                workbook = Workbook()
                ws = workbook.active
                ws.cell(row=1, column=1, value=Atitle_columns[0])
                for idx, title in enumerate(Atitle_columns[i:i + 8], start=2):
                    ws.cell(row=1, column=idx, value=title)
                for i, first_value in enumerate(first_column):
    ...
    def average(si):
        ...
        for i in range(len(si)):
            ...
            for i in range(len(unique_numbers)):
                element = unique_numbers[i]
                file1 = os.path.join(subfolder_path, f'{Size}-Damage-{element}.xlsx')
                file2 = os.path.join(subfolder_path, f'{Size}-Eqplas-{element}.xlsx')
                file3 = os.path.join(subfolder_path, f'{Size}-FS-{element}.xlsx')
                file4 = os.path.join(subfolder_path, f'{Size}-Size-{element}.xlsx')
                file5 = os.path.join(subfolder_path, f'{Size}-FSIZE-{element}.xlsx')
                file6 = os.path.join(subfolder_path, f'{Size}-DIS-{element}.xlsx')
                file7 = os.path.join(subfolder_path, f'{Size}-TRI-{element}.xlsx')

                Time, fracture_point, fracture_columns = damage_time(file1)
                delete_time = Time[fracture_point]
                # print(element, fracture_point, fracture_columns)

                # try:
                #     element_dis = crack(file6, delete_time, list(range(1, 9)))
                #     print(element, delete_time, element_dis)
                #     results.append([element, delete_time, element_dis])
                # except Exception as e:
                #     print(element)
                #     continue
                # output_file = f"results-{Size}-D-DIS.csv"
                ...
                try:
                    ...
                    SIZE4_ratio, SIZE4_value, SIZE4_ave, SIZE4_value_ratio = calculate_average2(file1, file2, file3,
                                                                                                ...
                                                                                                fracture_point,
                                                                                                fracture_columns)
                    # SIZE8_ratio, SIZE8_value, SIZE8_ave, SIZE8_value_ratio = calculate_average(file1, file2, file3,
                    #                                                                            file4,
                    #                                                                            fracture_point,
                    #                                                                            fracture_columns2)
                    # FSIZE4_ratio, FSIZE4_value, FSIZE4_ave, FSIZE4_value_ratio = calculate_average2(file1, file2, file3,
                    #                                                                                 file5,
                    #                                                                                 fracture_point,
                    #                                                                                 fracture_columns)
                    # FSIZE8_ratio, FSIZE8_value, FSIZE8_ave, FSIZE8_value_ratio = calculate_average(file1, file2, file3,
                    #                                                                                file5,
                    #                                                                                fracture_point,
                    #                                                                                fracture_columns2)
                    results_size.append([element, SIZE4_ratio, SIZE4_value])
                except Exception as e:
                    continue

            output_file = f"results-Size-{Size}.csv"  # Size
            with open(output_file, mode='w', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerow(
                    ["element", 'ratio', 'value'])
            ...
            results_size_array = np.array(results_size, dtype=float)[:, 1:]
            max_values2 = np.nanmax(results_size_array, axis=0)
            min_values2 = np.nanmin(results_size_array, axis=0)
            mean_values2 = np.nanmean(results_size_array, axis=0)
        ...
        print(f"Data has been saved to {stats_output_file}")
        return effsize
...
def Design_Arctan_R2(x_data, n_list, fs_list, atri_list, A_data, B_data):
    def arctan_model(x, a, b, c, d):
        return a * np.arctan(b * x + c) + d

    def fit_and_r2(x, y, label, p0):
        params, _ = curve_fit(arctan_model, x, y, p0=p0, maxfev=10000)
        y_pred = arctan_model(x, *params)
        # R²
        ss_res = np.sum((y - y_pred) ** 2)
        ss_tot = np.sum((y - np.mean(y)) ** 2)
        r2 = 1 - (ss_res / ss_tot)
        print(
            f"\n[{label}] Fitted parameters: a={params[0]:.6f}, b={params[1]:.6f}, c={params[2]:.6f}, d={params[3]:.6f}")
        print(f"R² = {r2:.6f}")
        return params, r2

    params_A, r2_A = fit_and_r2(x_data, A_data, "A", [8, 20, -5, 1])
    params_B, r2_B = fit_and_r2(x_data, B_data, "B", [3, 20, -5, 1])
    print(
        f"Fitted A(x) = {params_A[0]:.6f} *  DATAN({params_A[1]:.6f} * element_size {params_A[2]:.6f}) + {params_A[3]:.6f}")
    print(
        f"Fitted B(x) = {params_B[0]:.6f} *  DATAN({params_B[1]:.6f} * element_size {params_B[2]:.6f}) + {params_B[3]:.6f}")
    #
    x_smooth = np.linspace(0, 1, 30)
    fit_A_smooth = arctan_model(x_smooth, *params_A)
    fit_B_smooth = arctan_model(x_smooth, *params_B)
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.scatter(x_data, A_data, label='A Data', color='blue')
    plt.plot(x_smooth, fit_A_smooth, label='Fitted A', color='black')
    plt.title('Fitting A with arctan')
    plt.xlabel('x')
    plt.ylabel('A')
    plt.legend()
    plt.subplot(1, 2, 2)
    plt.scatter(x_data, B_data, label='B Data', color='green')
    plt.plot(x_smooth, fit_B_smooth, label='Fitted B', color='black')
    plt.title('Fitting B with arctan')
    plt.xlabel('x')
    plt.ylabel('B')
    plt.legend()
    plt.tight_layout()
    ...
    df_out = pd.DataFrame({
        's': x_smooth,
        'Designed A': fit_A_smooth,
        'Designed B': fit_B_smooth
    })
